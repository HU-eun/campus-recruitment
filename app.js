...
function renderLocationFilters() {
  ...
  let html = '';
  if (hot.length > 0) {
    html += '<div class="location-group-title">🔥 热门城市</div><div class="filter-tags" style="margin-bottom:8px">' + hot.map(l => filterTagHtml('location', l)).join('') + '</div>';
  }
  if (others.length > 0) {
    html += '<div class="location-group-title">📍 其他城市</div><div class="filter-tags">' + others.map(l => filterTagHtml('location', l)).join('') + '</div>';
  }
...
// ========== 简历版本管理 ==========
let resumeVersions = DB.get('resumeVersions', null);
let currentResumeVersion = 0;

if (!resumeVersions || !Array.isArray(resumeVersions) || resumeVersions.length === 0) {
  resumeVersions = [{ name: '默认简历', sections: JSON.parse(JSON.stringify(resumeSections)) }];
  DB.set('resumeVersions', resumeVersions);
}

function getCurrentResume() {
  return resumeVersions[currentResumeVersion] || resumeVersions[0];
}

function renderResumeVersionTabs() {
  const container = document.getElementById('resumeVersionTabs');
  if (!container) return;
  container.innerHTML = resumeVersions.map((v, idx) =>
    '<span class="resume-version-tab' + (idx === currentResumeVersion ? ' active' : '') + '" onclick="switchResumeVersion(' + idx + ')">' + v.name + '</span>'
  ).join('');
}

function switchResumeVersion(idx) {
  currentResumeVersion = idx;
  renderResumeVersionTabs();
  renderResume();
}

function addResumeVersion() {
  const name = prompt('请输入新版本名称：', '简历版本' + (resumeVersions.length + 1));
  if (!name) return;
  const current = getCurrentResume();
  resumeVersions.push({ name: name, sections: JSON.parse(JSON.stringify(current.sections)) });
  currentResumeVersion = resumeVersions.length - 1;
  DB.set('resumeVersions', resumeVersions);
  renderResumeVersionTabs();
  renderResume();
  showToast('已创建新版本：' + name);
}

function renameResumeVersion() {
  const current = getCurrentResume();
  const name = prompt('请输入新名称：', current.name);
  if (!name) return;
  resumeVersions[currentResumeVersion].name = name;
  DB.set('resumeVersions', resumeVersions);
  renderResumeVersionTabs();
...
function deleteResumeVersion() {
  if (resumeVersions.length <= 1) {
    showToast('至少保留一个简历版本');
    return;
  }
  const current = getCurrentResume();
  if (!confirm('确定删除简历版本「' + current.name + '」吗？')) return;
  resumeVersions.splice(currentResumeVersion, 1);
  currentResumeVersion = Math.min(currentResumeVersion, resumeVersions.length - 1);
  DB.set('resumeVersions', resumeVersions);
  renderResumeVersionTabs();
  renderResume();
  showToast('已删除');
...
// ========== 简历体检功能 ==========
const STOP_WORDS = new Set(['的','了','在','是','我','有','和','就','不','人','都','一','一个','上','也','很','到','说','要','去','你','会','着','没有','看','好','自己','这','那','他','她','它','我们','你们','他们','可以','能够','以及','或者','但是','因为','所以','如果','虽然','并且','而且','还是','就是','只是','只有','还有','等','等等','相关','进行','负责','工作','岗位','职位','要求','职责','任职','资格','条件','优先','经验','能力','熟悉','掌握','了解','具有','具备','良好','优秀','较强','一定','以上','学历','专业','本科','硕士','博士','应届','毕业生','实习生','实习','全职','兼职','薪资','待遇','福利','五险一金','周末','双休','加班','出差','地点','地址','公司','企业','单位','机构','集团','有限','股份','科技','信息','网络','软件','硬件','电子','机械','自动化','工程','工程师','助理','专员','主管','经理','总监','负责人','团队','部门','项目','产品','运营','市场','销售','客服','技术','研发','设计','测试','运维','数据','算法','前端','后端','全栈','移动端','安卓','苹果','ios','android','windows','linux','mac','web','app','api','ui','ux','ai','it','hr','oa','erp','crm','saas','paas','iaas','cloud','big','data','machine','learning','deep','neural','network','system','software','hardware','database','server','client','browser','application','development','programming','coding','testing','debugging','deployment','maintenance','support','analysis','design','management','marketing','sales','customer','service','product','project','team','leader','manager','director','engineer','developer','architect','consultant','specialist','analyst','designer','tester','operator','administrator','intern','graduate','fresh','experienced','senior','junior','mid','lead','principal','staff','fellow']);

function extractKeywords(text, topN) {
  if (!text) return [];
  topN = topN || 30;
  const freq = {};
  const enWords = text.toLowerCase().match(/[a-z][a-z0-9+#.]{1,}/g) || [];
  enWords.forEach(w => {
...
function getResumeFullText() {
  const current = getCurrentResume();
  return current.sections.map(s => s.content).join('\n');
}
...
function runResumeCheck() {
  ...
  jdKeywords.forEach(kw => {
    const isHit = resumeKeywords.some(rk => rk.includes(kw) || kw.includes(rk));
    if (isHit) matched.push(kw);
    else missed.push(kw);
  });
  const score = jdKeywords.length > 0 ? Math.round(matched.length / jdKeywords.length * 100) : 0;
  const suggestions = generateSuggestions(matched, missed, resumeText, jd);
  const interviewQuestions = generateInterviewQuestions(jd, matched, missed);
  renderCheckResult(score, matched, missed, suggestions, interviewQuestions);
}

function generateSuggestions(matched, missed, resumeText, jd) {
  const suggestions = [];
  if (missed.length > 0) {
    const topMissed = missed.slice(0, 5);
    suggestions.push({
      type: 'keyword',
      text: '简历中缺少JD高频关键词：' + topMissed.join('、') + '。建议在项目经历或技能部分补充这些关键词。'
    });
  }
  const hasNumbers = /\d+%|\d+倍|\d+个|\d+万|\d+千|提升|降低|增加|减少|优化/.test(resumeText);
  if (!hasNumbers) {
    suggestions.push({
      type: 'quantify',
      text: '简历中缺少量化成果。建议把"负责XX"改成"通过XX方法，将XX提升了XX%"，用数据证明成果。'
    });
  }
  const hasActionResult = /通过|利用|使用|采用|设计|开发|实现|完成|优化|解决/.test(resumeText);
  if (!hasActionResult) {
    suggestions.push({
      type: 'structure',
      text: '建议采用"动作+工具+结果"结构描述经历，例如："使用Python开发数据清洗脚本，将处理效率提升40%"。'
    });
  }
  if (matched.length > 0) {
    suggestions.push({
      type: 'strength',
      text: '已命中关键词：' + matched.slice(0, 8).join('、') + '。这些是你的优势，面试时可以重点展开。'
    });
  }
  suggestions.push({
    type: 'general',
    text: '投递前建议：①针对岗位调整简历顺序，把最相关的经历放前面；②一页纸原则，精简无关内容；③文件名用"姓名-岗位-学校"格式。'
  });
  return suggestions;
}

function generateInterviewQuestions(jd, matched, missed) {
  const questions = [];
  questions.push('请做一个1分钟的自我介绍，重点突出与这个岗位相关的经历。');
  questions.push('你为什么选择我们公司和这个岗位？你对这个行业有什么了解？');
  matched.slice(0, 3).forEach(kw => {
    questions.push('请详细讲讲你在「' + kw + '」方面的项目经验，遇到了什么挑战，怎么解决的？');
  });
  if (missed.length > 0) {
    questions.push('JD中提到「' + missed[0] + '」，你在这方面经验如何？如果入职后需要快速掌握，你会怎么做？');
  }
  questions.push('讲一个你团队合作中遇到冲突的例子，你是怎么处理的？');
  questions.push('你最大的缺点是什么？你正在如何改进？');
  questions.push('你有什么想问我们的？');
  return questions.slice(0, 8);
}

function renderCheckResult(score, matched, missed, suggestions, interviewQuestions) {
  const container = document.getElementById('resumeCheckResult');
  if (!container) return;
  let scoreColor = score >= 70 ? '#16a34a' : score >= 50 ? '#f59e0b' : '#dc2626';
  let scoreLabel = score >= 70 ? '匹配度良好' : score >= 50 ? '匹配度一般，建议优化' : '匹配度较低，需要大幅调整';
  let html = '<div style="border-top:1px solid #e2e8f0;padding-top:16px;margin-top:16px">';
  html += '<div style="text-align:center;margin-bottom:20px">';
  html += '<div class="check-score-circle" style="background:' + scoreColor + '">' + score + '分</div>';
  html += '<div style="font-size:14px;font-weight:600;color:' + scoreColor + ';margin-top:8px">' + scoreLabel + '</div>';
  html += '<div style="font-size:12px;color:#64748b;margin-top:4px">JD关键词命中 ' + matched.length + '/' + (matched.length + missed.length) + '</div>';
  html += '</div>';
  html += '<div style="margin-bottom:16px">';
  html += '<div style="font-size:14px;font-weight:600;color:#1e293b;margin-bottom:8px">📊 关键词分析</div>';
  if (matched.length > 0) {
    html += '<div style="margin-bottom:8px"><span style="font-size:12px;color:#16a34a;font-weight:500">✅ 已命中 (' + matched.length + ')：</span><br>';
    html += matched.map(k => '<span class="check-keyword-tag check-keyword-hit">' + k + '</span>').join('');
    html += '</div>';
  }
  if (missed.length > 0) {
    html += '<div><span style="font-size:12px;color:#dc2626;font-weight:500">❌ 缺失 (' + missed.length + ')：</span><br>';
    html += missed.map(k => '<span class="check-keyword-tag check-keyword-miss">' + k + '</span>').join('');
    html += '</div>';
  }
  html += '</div>';
  html += '<div style="margin-bottom:16px">';
  html += '<div style="font-size:14px;font-weight:600;color:#1e293b;margin-bottom:8px">💡 优化建议</div>';
  suggestions.forEach(s => {
    html += '<div class="check-suggestion-item">' + s.text + '</div>';
  });
  html += '</div>';
  html += '<div>';
  html += '<div style="font-size:14px;font-weight:600;color:#1e293b;margin-bottom:8px">🎤 面试预测问题</div>';
  interviewQuestions.forEach((q, i) => {
    html += '<div class="check-interview-q"><strong>Q' + (i+1) + '.</strong> ' + q + '</div>';
  });
  html += '</div>';
  html += '</div>';
  container.innerHTML = html;
...
function toggleResumeCheck() {
  const panel = document.getElementById('resumeCheckPanel');
  const arrow = document.getElementById('resumeCheckArrow');
...
function clearResumeCheck() {
  ...
  document.getElementById('resumeCheckResult').innerHTML = '';
}

function renderResume() {
  renderResumeVersionTabs();
  const current = getCurrentResume();
  document.getElementById('resumeSections').innerHTML = current.sections.map((s, idx) => `
...
function copyResumeSection(idx) {
  const current = getCurrentResume();
  navigator.clipboard.writeText(current.sections[idx].content).then(() => showToast('已复制到剪贴板'));
}

function copyResumeAll() {
  const current = getCurrentResume();
  const all = current.sections.map(s => `【${s.title}】\n${s.content}`).join('\n\n');
...
function renderDashboard() {
  ...
  document.getElementById('dashboardContent').innerHTML = `
    ...
    <div class="card">
      <h3 style="font-size:15px;font-weight:600;margin:0 0 16px">📈 投递漏斗 <span style="font-size:12px;color:#64748b;font-weight:400">各阶段数量与转化率</span></h3>
      ${funnelData.map((f, i) => {
        const prevCount = i > 0 ? funnelData[i-1].count : f.count;
        const rate = prevCount > 0 ? Math.round(f.count / prevCount * 100) : 100;
        const overallRate = funnelData[0].count > 0 ? Math.round(f.count / funnelData[0].count * 100) : 0;
        return `
        <div style="display:flex;align-items:center;gap:12px;margin-bottom:10px">
          <div style="width:120px;font-size:13px;color:#475569;text-align:right">${f.label}</div>
          <div style="flex:1;position:relative">
            <div style="height:32px;background:${f.color}20;border-radius:4px;display:flex;align-items:center;padding-left:10px;font-size:13px;font-weight:500;color:${f.color};min-width:40px;width:${Math.max(f.count/maxCount*100, 8)}%">${f.count}</div>
          </div>
          <div style="width:100px;text-align:right">
            ${i > 0 ? '<span style="font-size:11px;color:#64748b">环比 ' + rate + '%</span>' : '<span style="font-size:11px;color:#94a3b8">-</span>'}
            <br><span style="font-size:11px;color:#94a3b8">总体 ' + overallRate + '%</span>
          </div>
        </div>`;
      }).join('')}
      <div style="margin-top:12px;padding:10px;background:#f8fafc;border-radius:8px;font-size:12px;color:#64748b">
        💡 环比 = 本阶段人数 / 上一阶段人数；总体 = 本阶段人数 / 投递总数。如果某阶段环比低于30%，建议重点复盘该环节。
      </div>
    </div>
    <div class="card">
